...
class LightGCN_CausE(nn.Module):
    def __init__(self, config, dataset):
        super(LightGCN_CausE, self).__init__()
    ...
    def forward(self, batch):
        ...
        batch_disease_ctrl_emb = self.embed_disease_ctrl(torch.tensor(diseases))
        batch_drug_ctrl_emb = self.embed_drug_ctrl(torch.tensor(drugs))
        cf_loss_disease = torch.norm(batch_disease_all_embeddings - batch_disease_ctrl_emb, dim=1).mean()
        cf_loss_drug = torch.norm(batch_drug_all_embeddings - batch_drug_ctrl_emb, dim=1).mean()
        cf_loss = self.cf_pen * (cf_loss_disease + cf_loss_drug)

        loss = BCE_loss + cf_loss
