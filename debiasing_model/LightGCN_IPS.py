...
class LightGCN_IPS(nn.Module):
    def __init__(self, config, dataset):
        super(LightGCN_IPS, self).__init__()
    ...
    def forward(self, batch):
        ...
        dr_diseaseEmbedding, dr_drugEmbedding = torch.split(d_r_embeddings,
                                                            [self.n_diseases, self.n_drugs])
        ...
        labels = torch.FloatTensor(labels).to(scores.device)

        pos_count = torch.sum(labels)
        neg_count = len(labels) - pos_count
        pos_weight = torch.where(pos_count > 0, neg_count / (pos_count + 1e-8), torch.tensor(1.0, device=scores.device))

        mf_loss = F.binary_cross_entropy_with_logits(scores, labels, pos_weight=pos_weight.expand_as(scores))

        regularizer = 0.5 * (torch.norm(batch_disease_all_embeddings) ** 2 + torch.norm(batch_drug_all_embeddings) ** 2)
        reg_loss = self.decay * regularizer / len(diseases)

        BCE_loss = F.binary_cross_entropy_with_logits(scores, labels, pos_weight=pos_weight.expand_as(scores))

        loss = mf_loss + reg_loss + BCE_loss
